...
function matchesValueCondition(
	...
	// Scope to .metadata-property-value so we don't accidentally match the key-name input.
	if (propEl) {
		// Checkbox: read checked state directly
		const checkbox = propEl.querySelector<HTMLInputElement>(
			'.metadata-property-value input[type="checkbox"]'
		);
		if (checkbox) {
			const domVal = checkbox.checked ? 'true' : 'false';
			return conditions.includes(domVal);
		}

		// Text/number/date inputs and textareas
		// Only use DOM value if non-empty (empty multi-select search boxes fall through)
		const domInput = propEl.querySelector<HTMLInputElement | HTMLTextAreaElement>(
			'.metadata-property-value input:not([type="checkbox"]), .metadata-property-value textarea'
		);
...
export default class MetadataHider extends Plugin {
	 ...
	 */
	applyConditionalHiding(isRetry = false) {
		const activeFile = this.app.workspace.getActiveFile();
		const frontmatterLower = buildFrontmatterLower(
			activeFile ? this.app.metadataCache.getFileCache(activeFile)?.frontmatter : null
		);
		...
		// If containers or property rows haven't rendered yet, retry once.
		// (Happens when applyConditionalHiding is called before Obsidian finishes rendering.)
		if (!isRetry) {
			let foundAnyProps = false;
			containers.forEach(c => { if (c.querySelector('.metadata-property[data-property-key]')) foundAnyProps = true; });
			if (!foundAnyProps) {
				setTimeout(() => this.applyConditionalHiding(true), 500);
				return;
			}
		}
		...
		containers.forEach(container => {
			...
			container.querySelectorAll<HTMLElement>('.metadata-property[data-property-key]').forEach(prop => {
				const key = (prop.dataset?.propertyKey ?? '').toLowerCase();
				let matched = false;
			...
			if (metadataElement?.contains(target as Node)) {
				...
				setTimeout(() => {
					if (!this.isMetadataFocused) {
						metadataElement.classList.remove('is-active');
						// Re-evaluate value conditions now that is-active is gone so
						// conditional hiding takes effect immediately on blur.
...
function genAllCSS(plugin: MetadataHider): string {
	...
	// NOTE: These are placed as low-specificity fallbacks. The high-specificity overrides
	// are appended AFTER the hideEmptyEntry .is-active rule (see below) so they win
	// the specificity battle when both .mh-hide and .is-active apply.
	...
	// High-specificity overrides for DOM-managed .mh-hide / .mh-show so they beat the
	// .metadata-container.is-active rule (specificity 0,3,0) generated by hideEmptyEntry.
	// These use specificity (0,4,0) and appear after .is-active in source order.
	content.push(`.metadata-container .metadata-properties .metadata-property.mh-hide { display: none !important; }`);
	content.push(`.metadata-container .metadata-properties .metadata-property.mh-show { display: flex !important; }`);
